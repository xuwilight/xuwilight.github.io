...
下面将按照从简单到复杂的顺序依次介绍layout中常用的函数，包括 `Flatten`, `Coalesce`, `Composition`, `Complement`, `Division`, `Production`, `Inverse`。
...
首先将layout的shape和stride使用flatten进行展平。
...
- $\((1, s_1) : (d_0, d_1)\)$ 或 $\((s_0, 1) : (d_0, d_1)\)$。这两种情况下layout虽然是二维的，但是由于有一维shape=1，所以可以看成是1维的，因此可以直接合并成 $\(s_1 : d_1\)$ 或 $\(s_0 : d_0\)$。

- $\((s_0, s_1) : (d_0, d_1)\)$，其中 $\(d_1 = s_0 * d_0\)$。这种情况下如果按照column-major展开，所有元素的间隔都相同，因此可以把这两维合并成 $\((s_0 * s_1) : d_0\)$。

- $\((s_0, s_1) : (d_0, d_1)\)$。如果以上三种条件都不满足，需要保留原维度的shape和stride，即合并后还是 $\((s_0, s_1) : (d_0, d_1)\)$。

上面只描述了前两维的处理方法，前两维处理完成后需要继续循环遍历后面的维度，直到所有维度处理完毕。

**Python伪代码**

下面是python实现的coalesce核心代码，
...
对于两个函数f(x)和g(x)，f(g(x))称为他们的函数组合（composition）。
