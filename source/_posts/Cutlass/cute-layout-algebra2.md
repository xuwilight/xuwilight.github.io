...
在 CuTe 中，一个布局（Layout, 记为 $L$）本质上是一个数学函数：它将张量的逻辑坐标映射到物理内存的偏移量，即 `逻辑坐标 -> 物理偏移`。例如：知道矩阵的第 (i, j) 个元素，Layout 告诉你它在内存数组的第几个位置。

对于普通的数学函数，如果它是双射（Bijection），我们就能求出它的完全逆函数（Inverse, 记为 $L^{-1}$），此时 $L^{-1}(L(k)) = k$。

在布局代数的语境下，如果一个布局是完全连续且无重叠的（例如标准的行优先二维数组，步长严格等于其尺寸），我们可以直接求逆，从物理偏移反向找回逻辑坐标。也就是给定内存中的第 0, 1, 2 个连续元素，逆布局告诉你它们对应矩阵里的哪些 (i, j) 坐标。这种完全可逆的布局也被称为“紧凑（Compact）”布局。

但是，在实际的硬件和 AI 算子优化中，我们遇到的布局往往并不完美。它们可能包含空洞（如较长的步长 stride），或者重叠（如步长为 0，实现广播）。此时，标准的“完全逆”不存在，于是 CuTe 引入了**右逆**和**左逆**这两个广义的伪逆概念。

### Right-Inverse（右逆）

**右逆（记为 $L^\ddagger$）** 解决的是这样一个问题：“当我有一段物理内存是连续时，这些连续的内存对应着张量的哪些逻辑坐标？”因为我们要考虑的是“连续内存”而不一定是“所有内存”，所以右逆常用于识别张量中可向量化处理的数据块。
...
需要注意的是：右逆的定义域大小 $L^\ddagger$ 往往小于等于原始布局的大小 $L$，因为只有物理上连续的那部分才能构成右逆的有效输入。

#### 右逆的具体示例

下表展示了不同的布局 $L$ 及其对应的右逆 $L^\ddagger$。通过观察表格，可以直观感受到右逆的作用：**它剥离了原始布局中的“空洞”，只保留物理上连续的内存块，并将其在逻辑上重组成一个新的紧凑布局。**
...
在前面的定义中可以知道，逆布局的输入是物理索引，输出是逻辑坐标。但是一个布局的输出往往只有一个值，但是逻辑坐标一般都是多维的，所以 CuTe 中定义了一维的逻辑索引，而且逻辑索引默认是按照列主序排列的。

也就是说在 CuTe 中，无论你的 Layout 是行优先还是列优先，如果你输入一个一维的逻辑索引 (0, 1, 2, 3...)，CuTe 永远默认是以列优先（Column-major）的方式去遍历 Shape 的。

以上面第二个例子为例，对于 Shape 为 (4, 8)（4行8列）的矩阵，逻辑索引 $c$ 是这样分配的：

*   **第 0 列：** $c = 0, 1, 2, 3$ （分别对应坐标 (0,0), (1,0), (2,0), (3,0)）
*   **第 1 列：** $c = 4, 5, 6, 7$ （分别对应坐标 (0,1), (1,1), (2,1), (3,1)）
*   ...依此类推。

L = (4,8) : (8,1) 的步长 (Stride) 是 (8,1)，说明它是行优先的物理存储（行与行之间差 8，列与列之间连续，差 1）。

把逻辑索引 0~31 代入这个 Layout，可以得到它们在物理内存中的真实位置（偏移量）：

| 逻辑索引 $c$ | 矩阵坐标 (i, j) | 物理偏移量 $k = i \times 8 + j \times 1$ |
| :---: | :---: | :---: |
| **0** | (0, 0) | **0** |
| **1** | (1, 0) | **8** |
| **2** | (2, 0) | **16** |
...
可以看到，从正向的角度看，如果按**逻辑索引** (0, 1, 2, 3, 4...) 顺序去读，物理内存是**不连续**的，内存地址在 `0 -> 8 -> 16 -> 24 -> 1 -> 9...` 这样变化。

右逆 (Right-Inverse) 的本质是：输入连续的物理内存偏移量 $k$，输出对应的逻辑索引 $c$。

行优先矩阵在“行方向”上物理内存是连续的。所以，如果我们在物理内存中按顺序走 (k = 0, 1, 2, 3, 4...)，逻辑索引则会变成下面这样。

| 连续的物理偏移量 $k$ | 对应的逻辑索引 $c$ | 规律观察 |
| :---: | :---: | :---: |
| **0** | **0** | (第0行的第0个元素) |
| **1** | **4** | (第0行的第1个元素，逻辑上跨了4) |
| **2** | **8** | (第0行的第2个元素) |
| **3** | **12** | ... |
| ... | ... | ... |
| **7** | **28** | (第0行的最后1个元素) |
...
现在，需要构造一个新的 Layout，这个新 Layout 就像一个数学函数 $f(k) = c$，它能通过 `0, 1, 2...`这样的输入，得到 `0, 4, 8... 28, 1, 5, 9...` 这样的输出。

*   观察连续段 (Shape的第一维和Stride的第一维)：

    物理地址每增加 1（比如 0到1，1到2），输出结果就会增加 4。这种连续规律有 8 次（因为一行有 8 个元素）。所以，右逆 Layout 的第一维 Shape 是 8，第一维 Stride 是 4。 -> (8, ...): (4, ...)

*   观察跳跃段 (Shape的第二维和Stride的第二维)：

    当物理地址跨过一行（增加 8，比如从 0 跳到 8），输出结果从 0 变成了 1，相当于增加了 1。总共有4 行。所以，右逆 Layout 的第二维 Shape 是 4，第二维 Stride 是 1。 -> (..., 4): (..., 1)

组合起来，这个反向映射的 Layout 就是 $(8, 4) : (4, 1)$。

1.  为什么是 8 行 4 列？

    这里右逆的 Shape (8, 4) 已经不再是原来矩阵的“长宽”概念了。它表示的是物理内存的分块结构：物理内存中最内层连续的一块有 8 个元素（原矩阵的一行），共有这样的 4 块（原矩阵的4行）。

2.  为什么 Stride 是 (4, 1)？

    它告诉你，在物理内存上连续走 1 步，对应的“逻辑进度”会快进 4 步；在物理内存上跳过 8 步（进入下一块），对应的“逻辑进度”只前进了 1 步。

因为正向 Layout 逻辑默认是列优先的（列距4），而物理存储是行优先的（行长8）；所以反过来推算时，连续物理内存（长度8）造成的逻辑步长就是4，这就是 `(8,4):(4,1)` 的由来。
...
假设我们要将张量 A 拷贝到张量 B。如果不做优化，我们可能会一个元素一个元素地拷贝。但 GPU/CPU 支持向量化拷贝指令，一次可以加载 4 个或 8 个 Float 数据——前提是这 4 个数据在物理内存里是连续且对齐的。
...
左逆和右逆有什么区别？在数学定义上，它们看起来极其相似，但含义截然不同：

*   右逆 $\mathbf{L}^\ddagger$：满足 $\mathbf{L}(\mathbf{L}^\ddagger(k)) = k$
    *  视角：站在物理内存的角度。
    *  问题：给定一个连续的物理地址序列 $k = 0, 1, 2...$，它们对应哪些逻辑坐标？（用来找连续的数据块）
*   左逆 $\mathbf{L}^\dagger$：满足 $\mathbf{L}^\dagger(\mathbf{L}(k)) = k$ （对于单射布局）
    *   视角：站在原布局的角度。
    *   问题：假设原布局 $\mathbf{L}$ 把逻辑坐标 $k$ 映射到了某个物理地址（假设是 $P$），那么我把 $P$ 喂给左逆 $\mathbf{L}^\dagger$，左逆必须能准确地把 $P$ **还原**回逻辑坐标 $k$。也就是说只要是原布局产生过的物理地址，左逆都能告诉你它原来是第几个逻辑元素。
...
我们定义的数据布局 $\mathbf{A}$ 是任意的，而硬件指令布局 $\mathbf{T}$ 是固定的。我们需要判断：“这个特定的 TMEM 指令，是否能安全地访问我当前定义的张量布局 $\mathbf{A}$ ？”

我们计算复合布局 $A^\dagger \circ T$，将硬件指令要求的物理偏移 $\mathbf{T}(i)$ 作为输入，利用张量的左逆 $\mathbf{L}^\dagger$，计算出这个物理偏移对应在张量逻辑坐标中的位置。如果计算成功，说明硬件指令访问的物理偏移完全落在这个张量的有效范围内，则该指令是可容许（Admissible）的。
