...
`cp.async` 指令在 Hopper 架构上进行了拓展，新增了 `cp.async.bulk` 和 `cp.async.bulk.tensor` 两种，也就是能使用 Tensor Memory Accelerate （TMA）进行异步拷贝的指令。
...
与 Ampere 架构上的 `cp.async` 不同的是，TMA 只需要一个线程就可以启动异步拷贝，而且可以拷贝大块的数据（bulk），或者多维的数据（bulk.tensor）。
