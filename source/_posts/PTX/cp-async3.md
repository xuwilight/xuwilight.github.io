...
由于 Tensor 是多维结构，所以对 Tensor 进行拷贝时限制比较多。主要的限制如下：
...
启动一个异步复制操作，从一个内存空间复制 tensor 到另一个内存空间。方向可以是 global -> shared::cta，global -> shared::cluster，shared::cta -> global，不支持 shared::cluster和shared::cta 之间的传输。
...
Tensor 的维度由 .dim 修饰符指定。向量操作数 tensorCoords 指定全局内存中 tensor 的起始坐标。tensorCoords 中各个张量坐标的类型为 .s32。向量参数 tensorCoords 的格式取决于指定的 .load_mode，如下所示：
...
创建 tensor map 需要许多参数。其中包括指向全局内存中数组的指针、数组的大小（以元素数量为单位）、行与行之间的步长（以字节为单位）、共享内存的大小（以元素数量为单位）。

具体如下，下面代码展示了通过 CUDA API 创建 tensor map的过程。创建的 tensor map 用于描述一个大小为 GMEM_HEIGHT x GMEM_WIDTH 的二维行主数组。
...
下面是使用 TMA 拷贝一维 Tensor 数据的代码，使用的是 `cp.async.bulk.tensor` 指令，基本过程和前面介绍的 `cp.async.bulk` 指令相同，只不过这里需要额外传入 TMA 描述符。
...
下面是使用 TMA 拷贝二维 Tensor 的代码。这里需要确定二维坐标 crd0 和 crd1。
...
在数据传输过程中，TMA 会根据 swizzle 模式对数据进行 shuffle。如下图所示，图中每个色块代表连续的 16 bytes，也就是 4 个 bank，所以一行有128 bytes，32 个 bank。

TMA 支持 4 种 swizzle 类型，分别是 None，32B，64B 和 128B。图中从左到右依次是 None，128B，64B 和 32B 对应的swizzle 模式。
...
上面的宽度限制是在一个 swizzle pattern 内的限制，如果 shared memory 很大的话一般需要用多个 swizzle pattern 处理。
...
共享内存块的内部维度必须满足规定的大小要求。如果不满足这些要求会报错。
...
假如 global memory 的大小是 1024 × 1024，每一行的元素分别是 0 - 1023，然后使用 2D TMA 拷贝数据到 shared memory。我们使用不同的 swizzle pattern 进行拷贝，并打印 (0, 0) 位置 block 的拷贝结果。
...
当使用 TMA 把数据从 shared memory 拷贝到 global memory 上的时候需要使用 group 完成机制。
...
也就是使用 .read 的时候只需要确保读到数据就行了，不会等待数据完成写入。
...
对多维 Tensor 类型进行异步拷贝是 TMA 最常用的功能之一。多维 Tensor 是抽象出来的数据结构，需要使用 shape 和 stride 进行描述。

使用 TMA 加载多维 Tensor 时需要提前设置一个 tensor map 描述符，里面记录了 global memory 上的 Tensor 的基本信息，包括地址，shape，stride，数据类型等，同时也会记录 shared memory 相关的信息，包括大小，swizzle 类型等。通过这些信息 TMA 硬件可以直接根据某一块数据的具体坐标来确定要拷贝什么数据，以及怎么拷贝。

因为 shared memory 上的数据访问很容易会触发 bank conflicts，导致性能下降。因此，Hopper 上通过 swizzle 机制来避免 bank conflicts。这里的 swizzle 机制不仅对 TMA 有效，对后面的 wgmma 指令也有效。

最后，由于 TMA 是异步拷贝，需要通过完成机制来确保异步拷贝的结果对其他线程可见。因此 TMA 有两种完成机制。当从 global memory 拷贝到 shared memory 时，使用 mbarrier 进行细粒度的完成控制。当从 shared memory 拷贝到 global memory 上的时候则使用 async-group 进行完成控制。