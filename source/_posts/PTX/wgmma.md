...
tags: [CUDA, PTX, wgmma, Tensor Core]
categories: [PTX 学习笔记]
...
<div align="center">
    <img src="/assets/wgmma/wgmma_A.png" width="100%" height="auto" alt="wgmma_a"><br>
    <small>wgmma A</small>
</div>
<br>
...
<div align="center">
    <img src="/assets/wgmma/wgmma_D.png" width="100%" height="auto" alt="wgmma_d"><br>
    <small>wgmma D</small>
</div>
<br>
...
如果是 half 类型，128B swizzle 在 K-major 下的 layout 为 `Sw<3,3,3> o (8,64):(64,1)`，如下图所示。
...
使用 none swizzle pattern k-major 进行 tiling，会得到下图这样。为了方便起见，下图已经把 fp16 recast 到 128bit 了，所以 smem 的 shape 变成了 16\*8。

<div align="center">
    <img src="/assets/wgmma/none_swizzle_k_tile.png" width="40%" height="auto" alt="swizzle"><br>
...
<div align="center">
    <img src="/assets/wgmma/32B_k_tile.png" width="40%" height="auto" alt="swizzle"><br>
...
使用 none swizzle pattern MN-major 进行分区得到下面的 layout。 为了方便起见，下图已经把 fp16 recast 到 128bit 了，所以 smem 的 shape 变成了 16\*16。

<div align="center">
    <img src="/assets/wgmma/none_swizzle_mn_tile.png" width="80%" height="auto" alt="swizzle"><br>
...
<div align="center">
    <img src="/assets/wgmma/128B_mn_tile.png" width="80%" height="auto" alt="swizzle"><br>
...
<div align="center">
    <img src="/assets/wgmma/lbo.png" width="100%" height="auto" alt="swizzle"><br>
...
- K-major 时：
    ...
    - 不管有没有 swizzle，SBO 都是前 8 行到后 8 行的 offset。
...
<div align="center">
    <img src="/assets/wgmma/sbo.png" width="100%" height="auto" alt="swizzle"><br>
...
- 第二个图是 K-major 32B swizzle，LBO 是两列之间的 offset。因为 wgmma fp16 在 K 方向固定是 16，也就是一个 wgmma 只会计算 128bit 下 K 方向的两列，因此所有的 swizzle 下，两列间 offset 都是 1。SBO 是前 8 行到后 8 行的 offset，这里是 16。

- 第三个图是 MN-major 下 none swizzle。这里跟其他情况不同，LBO 是 前 8 列到后 8 列，等于 128。SBO 是第一行到第二行的 offset，等于 8。

- 第四个图是 MN-major 下 128B swizzle。这种情况下 LBO 和 swizzle pattern 的宽度有关。128B 是前 8 行到后 8 行，等于 64。SBO 是前 8 列到后 8 列，等于 128。
...
<div align="center">
    <img src="/assets/wgmma/canonical_layout.png" width="100%" height="auto" alt="swizzle"><br>
...
3. stride dimension byte offset 的大小，14 位，占用 16 位。

4. matrix base offset，默认为 0，3 位，占用 8 位。

5. swizzling mode，2 位，占用 8 位。0 没有 swizzle，1 是 128B swizzle，2 是 64B swizzle，3 是 32B swizzle。

<div align="center">
    <img src="/assets/wgmma/wgmma_desc.png" width="100%" height="auto" alt="swizzle"><br>
...
当 A 是 K-major 时。因为是 none-swizzle。所以此时 swizzle 的基本 layout 是 `(8,1):(1,1)`。其中，一个元素是 128bit。
...
A 和 B 加载完成后，初始化两个描述符。变量按照上面分析的写。
...
当 A 是 M-major，128B-swizzle 时，pattern 的 layout 是 `Sw<3,0,3> o (8,8):(1,8)`，一个元素 128bit。
...
当矩阵 A 的数据是 column-major，也就是 M-major。由于数据在列方向是连续的，所以 ldmatrix 指令需要根据每一列的首地址加载一列中的 128 bits 的元素。

但是按列加载的数据与 mma 需要的分布不同，所以 ldmatrix 还需要打开 trans，将加载后的数据进行转置分布，这样就和 mma 指令需要的 A 矩阵的分布相同了。

__矩阵 B__

矩阵 B 的 shape 是 K\*N。当 B 矩阵中的数据是 row-major 时，可以称为 N-major。
